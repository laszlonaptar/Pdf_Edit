...
# ---------- Drive & Storage diagnosztika ----------
@app.get("/api/drive_probe")
async def drive_probe():
    # 1) lokális írás próba tartós könyvtárba
    try:
        test_local_name = f"probe_{uuid.uuid4().hex[:6]}.txt"
        test_local_path = GEN_DIR / test_local_name
        test_bytes = f"probe at {datetime.utcnow().isoformat()}".encode("utf-8")
        test_local_path.write_bytes(test_bytes)
        local_ok = True
    except Exception as e:
        return JSONResponse({"ok": False, "stage": "local_write", "error": repr(e)}, status_code=500)

    # 2) Drive feltöltés próba
    try:
        fid = drive_upload_bytes(test_local_name, test_bytes, "text/plain")
        return JSONResponse({
            "ok": True,
        ...
        return JSONResponse({"ok": False, "stage": "drive_upload", "error": repr(e)}, status_code=500)

# ---------- Health ----------
@app.get("/healthz")
async def healthz():
    return {"ok": True, "time": datetime.utcnow().isoformat(), "data_dir": str(DATA_DIR)}
