# main.py — FastAPI app: Excel page-setup -> PDF via LibreOffice (no ReportLab)
...
# ---- Optional: PIL for rendering Beschreibung szöveg képpé (Excel cellablokkba)
try:
...
# --- Beschreibung blokk: A6–G15 (igény szerint módosítható) ---
...
def _set_column_widths_for_print(ws, pos):
    widths = {
        pos["name_col"]: 18.0,
        pos["vorname_col"]: 14.0,
        pos["ausweis_col"]: 14.0,
        pos["beginn_col"]: 10.0,
        pos["ende_col"]: 10.0,
        pos["stunden_col"]: 12.0,
    }
    if "vorhaltung_col" in pos and pos["vorhaltung_col"]:
        widths[pos["vorhaltung_col"]] = 28.0
...
def build_workbook(datum, bau, basf_beauftragter, beschreibung, break_minutes, workers):
    ...
    try:
        ...
        # Ne középre igazítsuk (különben vizuálisan szűkebbnek tűnik)
...
async def generate_pdf(
    ...
    if not _has_soffice():
        return Response(
            content="LibreOffice (soffice) nicht gefunden. Bitte installieren Sie LibreOffice auf dem Server.",
    ...
    # Excel -> PDF: a Calc a sheet page setupját használja
    with tempfile.TemporaryDirectory() as tmpdir:
        xlsx_path = os.path.join(tmpdir, f"ln_{uuid.uuid4().hex[:8]}.xlsx")
        wb.save(xlsx_path)

        # Használjuk a filter paramétert, hogy 1×1 oldalra illesszen és lapbeállításokat vegye át
        ...
        cmd = [
            ...
            "--nofirststartwizard",
            "--convert-to",
            f"pdf:calc_pdf_Export:{filter_data}",
            "--outdir",
            tmpdir,
            xlsx_path,
        ]
        try:
            res = subprocess.run(cmd, check=True, stdout=subprocess.PIPE, stderr=subprocess.PIPE, timeout=180)
            # Kapott PDF ugyanabban a mappában, azonos alapnévvel
            pdf_path = xlsx_path[:-5] + ".pdf"
            with open(pdf_path, "rb") as f:
                pdf_bytes = f.read()
        except subprocess.CalledProcessError as e:
            msg = f"LibreOffice conversion failed.\nSTDOUT:\n{e.stdout.decode('utf-8','ignore')}\n\nSTDERR:\n{e.stderr.decode('utf-8','ignore')}"
            return Response(content=msg, media_type="text/plain", status_code=500)
        except Exception as e:
            return Response(content=f"LibreOffice error: {repr(e)}", media_type="text/plain", status_code=500)
